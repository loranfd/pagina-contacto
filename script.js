...
async function obtenerFilaPorId(id) {
  try {
    const params = new URLSearchParams({ action: 'getRow', id });
    const response = await fetch(urlApi, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: params
...
async function eliminarDefinitivo(id) {
  try {
    const params = new URLSearchParams({ action: 'deleteRow', id });
    const response = await fetch(urlApi, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: params
  ...
  const btn = crearBoton({
    ...
    onClick: async function () {
      ...
      try {
        ...
        // Actualizar SOLO el botón Llamado
        ...
        // Actualizar SOLO el botón Respondido (por si cambia el estado)
        ...
        // NO tocar otras celdas como td-seguimiento (documentación)
        
      } catch (e) {
        alert('Error al marcar Llamado: ' + e.message);
        // Restaurar el botón en caso de error
        this.style.backgroundColor = esLlamado ? colorVerde : colorRojo;
        this.style.border = `2px solid ${esLlamado ? bordeVerde : bordeRojo}`;
...
function mostrarOpcionesRespondido(td, id) {
  ...
  ['Sí', 'No'].forEach(valor => {
    ...
    btn.addEventListener('click', async () => {
      ...
      // Actualizar botón Respondido inmediatamente con el nuevo estado
      if (tdRespondido) {
        tdRespondido.innerHTML = '';
      ...
      try {
        ...
        // Si marca "Sí", también marcar Llamado como "Sí"
        if (valorNorm === 'si') {
        ...
        // Actualizar botón Llamado
        if (tdLlamado) {
          tdLlamado.innerHTML = '';
          tdLlamado.appendChild(crearBotonLlamado(id, filaData['Llamado']));
        }
        
        // NO tocar td-seguimiento (documentación)
        
      } catch (e) {
        alert('Error al actualizar Respondido: ' + e.message);
      }
    });
...
function crearBotonPrioridad(id, prioridad) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    estrella.addEventListener('click', async () => {
      ...
      estrella.style.animation = 'prioridadPulse 0.4s ease';
      setTimeout(() => { estrella.style.animation = ''; }, 400);
      ...
      try {
        ...
        // CORRECCIÓN: Verificar que tdEstado existe antes de modificarlo
...
function generarMensajeHTMLVillasCorreo2(nombreCompleto) {
  ...
  return `
    ...
    <p>Tal y como comentamos, y como habrá podido ver en el dossier que le enviamos previamente, se trata de un <strong>proyecto singular</strong> en un enclave privilegiado, donde cada vivienda combina <strong>diseño exclusivo</strong>, <strong>sostenibilidad certificada</strong> y unas <strong>vistas al mar</strong> que permanecerán siempre despejadas.</p>
    <p>Por la singularidad del proyecto y la demanda que estamos recibiendo, le animamos a <strong>actuar pronto</strong> para no dejar pasar la oportunidad de disfrutar de su nueva vivienda junto al mar.</p>
    <p>Nos encantaría acompañarle en el siguiente paso, ya sea <strong>realizando una reserva formal</strong> —lo que le otorgaría prioridad para asegurar la vivienda que elija— o concertando una <strong>reunión presencial</strong> para conocer el proyecto más de cerca.</p>
    <p>Quedo a su disposición para coordinar lo que le resulte más cómodo.</p>
    <p><strong>Un cordial saludo,</strong></p>
  `;
}
